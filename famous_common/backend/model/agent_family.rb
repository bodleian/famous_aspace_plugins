AgentFamily.include(AuthorityArks)

AgentFamily.class_eval do
  ...
  def assimilate(victims)
    ...
    begin
      victims.reject {|v| (v.class == self.class) && (v.id == self.id)}.each do |victim|
        AuthorityArk.filter(:agent_family_id => victim.id).update(:agent_family_id => self.id, :system_mtime => Time.now)
      end
