...
Subject.class_eval do
  ...
  def assimilate(victims)
    ...
    # Transfer Authority ARKs when merging subjects
    begin
      victims.reject {|v| (v.class == self.class) && (v.id == self.id)}.each do |victim|
        AuthorityArk.filter(:subject_id => victim.id).update(:subject_id => self.id, :system_mtime => Time.now)
      end
